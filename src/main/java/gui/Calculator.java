		...
		switch (value) {
			case "=" -> {
				try {
					...
					switch (this.operator) {
						case "+" -> {
							result = Integer.parseInt(this.number1) + Integer.parseInt(this.number2);
							this.displayText.setText(Integer.toString(result));
						}
						case "-" -> {
							result = Integer.parseInt(this.number1) - Integer.parseInt(this.number2);
							this.displayText.setText(Integer.toString(result));
						...
						case "/" -> {
							double r;
							try {
								r = Double.parseDouble(this.number1) / Double.parseDouble(number2);
								this.displayText.setText(Double.toString(r));
							} catch (ArithmeticException e) {
								this.displayText.setText("INF");
							}
						}

					}
				}catch (NumberFormatException e) {
					this.displayText.setText("ERROR");
				}
			...
			default -> {
				...
				if(value.chars().allMatch(Character::isDigit)){
					if(this.operator == null) this.number1 = this.number1 + value;
					else this.number2 = this.number2 + value;
